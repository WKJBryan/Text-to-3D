...
Keep it simple and focused on the specific object's needs."""
    ...
    def _extract_parameters_from_user_response(self, user_message: str) -> Dict:
        """Extract parameter updates from user's natural language response"""
        extracted = {}
        message_lower = user_message.lower()
        
        # Parameter name mappings - maps user terms to actual parameter names
        parameter_mappings = {
            # Dimensions
            'thickness': ['thickness', 'thick', 'wall thickness', 'wall_thickness'],
            ...
            'radius': ['radius', 'rad'],
            'bore_diameter': ['bore', 'hole', 'center hole', 'bore diameter'],
            'outer_radius': ['outer radius', 'outside radius', 'outer_radius'],
            'inner_radius': ['inner radius', 'inside radius', 'inner_radius'],
            
            # Gear-specific
            'teeth': ['teeth', 'tooth', 'cogs'],
            'module': ['module', 'mod'],
            'pressure_angle': ['pressure angle', 'pressure_angle'],
            
            # Spring-specific  
            'coils': ['coils', 'turns', 'wraps'],
            'wire_diameter': ['wire diameter', 'wire_diameter', 'wire thickness'],
            
            # Ring-specific
            'band_width': ['band width', 'band_width', 'band thickness'],
        ...
        # Numerical extraction patterns - more comprehensive
        number_patterns = [
            r'(\d+(?:\.\d+)?)\s*mm',  # 20mm, 5.5mm
            r'(\d+(?:\.\d+)?)\s*cm',  # 2cm, 1.5cm  
            r'(\d+(?:\.\d+)?)\s*inches?',  # 2inch, 1.5inches
            r'(\d+(?:\.\d+)?)\s*degrees?',  # 45degrees, 20.5degree
            r'(\d+(?:\.\d+)?)\s*deg',  # 45deg
            r'(\d+(?:\.\d+)?)\s*(?:teeth|tooth)',  # 20teeth, 24tooth
            r'(\d+(?:\.\d+)?)\s*(?:coils?|turns?)',  # 10coils, 5turns
            r'(\d+(?:\.\d+)?)(?=\s|$)',  # standalone numbers: "20" "5.5"
        ]
        
        # Extract all numbers with potential units
        found_numbers = []
        ...
        print(f"Found numbers in user message: {found_numbers}")
        
        # Context-based parameter assignment
        for param_name, keywords in parameter_mappings.items():
            for keyword in keywords:
                if keyword in message_lower:
                    # Look for numbers near this keyword
                    keyword_pattern = rf'{re.escape(keyword)}\s*[\s:]*(\d+(?:\.\d+)?)'
                    ...
                    if match:
                        try:
                            ...
                            print(f"Extracted {param_name} = {value} from keyword '{keyword}'")
                            break
                        except ValueError:
                            continue
                    
                    # Also check reverse pattern: number before keyword
                    reverse_pattern = rf'(\d+(?:\.\d+)?)\s*mm?\s*{re.escape(keyword)}'
                    reverse_match = re.search(reverse_pattern, message_lower)
                    
                    if reverse_match:
                        try:
                            value = float(reverse_match.group(1))
                            extracted[param_name] = value
                            print(f"Extracted {param_name} = {value} from reverse pattern with '{keyword}'")
                            break
                        except ValueError:
                            continue
        
        # Handle special cases and context inference
        if found_numbers and not extracted:
            # If we found numbers but no clear parameter mapping, use context
            self._infer_parameters_from_context(message_lower, found_numbers, extracted)
        
        # Handle unit conversions
        if 'cm' in message_lower:
            for param, value in extracted.items():
                if any(dim in param for dim in ['thickness', 'height', 'width', 'diameter', 'radius']):
                    extracted[param] = value * 10  # Convert cm to mm
                    print(f"Converted {param} from {value}cm to {extracted[param]}mm")
        
        # Handle boolean features (yes/no responses)
        self._extract_boolean_features(message_lower, extracted)
        
        return extracted

    def _infer_parameters_from_context(self, message: str, numbers: List[float], extracted: Dict):
        """Infer parameters based on context when no explicit mapping found"""
        if not self.detected_object or not numbers:
            return
        
        # Object-specific inference rules
        if self.detected_object == 'gear':
            if len(numbers) == 1:
                value = numbers[0]
                if 'thick' in message:
                    extracted['thickness'] = value
                elif 'teeth' in message or 'tooth' in message:
                    extracted['teeth'] = int(value)
                elif 'bore' in message or 'hole' in message:
                    extracted['bore_diameter'] = value
                elif 'module' in message:
                    extracted['module'] = value
                    
        elif self.detected_object in ['ring', 'band']:
            if len(numbers) >= 2:
                # First number likely outer radius, second inner radius
                extracted['outer_radius'] = max(numbers[:2])
                extracted['inner_radius'] = min(numbers[:2])
            elif len(numbers) == 1:
                value = numbers[0]
                if 'height' in message or 'thick' in message:
                    extracted['height'] = value
                elif 'outer' in message or 'outside' in message:
                    extracted['outer_radius'] = value
                elif 'inner' in message or 'inside' in message:
                    extracted['inner_radius'] = value
                    
        elif self.detected_object in ['cup', 'mug', 'container']:
            if len(numbers) >= 2:
                # Likely height and radius/diameter
                if any(h in message for h in ['height', 'tall', 'high']):
                    extracted['height'] = max(numbers[:2])
                    extracted['radius'] = min(numbers[:2])
                else:
                    extracted['height'] = numbers[0]
                    extracted['radius'] = numbers[1]
            elif len(numbers) == 1:
                value = numbers[0]
                if 'height' in message or 'tall' in message:
                    extracted['height'] = value
                elif 'radius' in message or 'diameter' in message:
                    extracted['radius'] = value / 2 if 'diameter' in message else value
        
        print(f"Context inference for {self.detected_object}: {extracted}")

    def _extract_boolean_features(self, message: str, extracted: Dict):
        """Extract yes/no feature preferences"""
        
        # Handle negative responses
        negative_phrases = ['no', 'without', "don't want", "don't need", 'none', 'skip']
        positive_phrases = ['yes', 'with', 'want', 'need', 'add', 'include']
        
        feature_keywords = {
            'has_handle': ['handle'],
            'has_lid': ['lid', 'cover', 'top'],
            'has_holes': ['holes', 'hole'],
            'has_pattern': ['pattern', 'patterns', 'decoration'],
            'has_decorative_bands': ['band', 'bands', 'decorative band'],
            'has_keyway': ['keyway', 'key slot'],
            'has_grooves': ['groove', 'grooves'],
        }
        ...
        for feature, keywords in feature_keywords.items():
            for keyword in keywords:
                if keyword in message:
                    ...
                    end = min(len(message), keyword_index + len(keyword) + context_window)
                    context = message[start:end]
                    
                    # Check for negative context
                    if any(neg in context for neg in negative_phrases):
                        extracted[feature] = False
                        print(f"Extracted {feature} = False from context '{context}'")
                    # Check for positive context  
                    elif any(pos in context for pos in positive_phrases):
                        extracted[feature] = True
                        print(f"Extracted {feature} = True from context '{context}'")
    ...
    def chat(self, user_message: str) -> Dict:
        ...
        if self.exchange_count == 1:
         ...
         # STEP 2: Extract and update parameters from user response (THIS IS THE NEW ADDITION)
        if self.exchange_count > 1:
            new_params = self._extract_parameters_from_user_response(user_message)
            if new_params:
                print(f"Updating parameters: {new_params}")
                self.extracted_parameters.update(new_params)
                print(f"Current parameters: {self.extracted_parameters}")
